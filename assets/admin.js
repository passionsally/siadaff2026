  ...
  submissionRows.innerHTML = rows.map((row) => {
    ...
    return `
      <tr>
        ...
        <td>${escapeHtml(row.age_group)}<br><span class="muted">${escapeHtml(row.production_type)} / ${escapeHtml(row.runtime_or_size)}</span>${row.business_registration_number ? `<br><span class="muted">사업자 ${escapeHtml(row.business_registration_number)}</span>` : ""}${row.business_registration_file_url ? `<br><a class="file-link" href="${escapeHtml(row.business_registration_file_url)}" target="_blank" rel="noreferrer">사업자등록증 보기</a>` : ""}</td>
...
function downloadCsv() {
  ...
  const headers = activeTab === "submissions"
    ? ["created_at", "receipt_no", "category", "title_ko", "title_en", "age_group", "production_type", "business_registration_number", "business_registration_file_path", "name", "phone", "email", "status", "admin_memo"]
