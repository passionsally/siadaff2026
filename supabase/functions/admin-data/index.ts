...
Deno.serve(async (request) => {
  ...
  if (request.method !== "GET") {
    return json({ error: "GET 요청만 허용합니다." }, 405, origin);
  }
  ...
  const [submissionsResult, sponsorResult] = await Promise.all([
    supabase
      .from("submissions")
      .select(
        "receipt_no,category,title_ko,title_en,work_title,age_group,production_type,runtime_or_size,ai_used,ai_description,synopsis_ko,synopsis_en,synopsis,youtube_url,instagram_url,tiktok_url,name,phone,email,status,created_at,updated_at",
      )
    ...
    supabase
      ...
      .select(
        "inquiry_no,organization_name,organization_type,contact_name,position_title,phone,email,interest_type,budget_range,message,marketing_consent,status,created_at,updated_at",
      )
      .order("created_at", { ascending: false })
      .limit(500),
  ]);
